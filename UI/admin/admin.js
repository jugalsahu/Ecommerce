...
const fetchPayments = async () => {
    try {
        ...
        for (let payment of data.items) {
            ...
            totalSeals = totalSeals + (payment.amount / 100)
...
`
        ...
        const totalSealsE1 = document.getElementById("total-sales")
        const totalFeeE1 = document.getElementById("total-fee")
        const totalTaxE1 = document.getElementById("total-tax")
        totalSealsE1.innerHTML = '₹' + Math.round(totalSeals).toLocaleString() //toLocalString() used for comma (,) in value
        totalFeeE1.innerHTML = '₹' + Math.round(totalFee).toLocaleString()
        totalTaxE1.innerHTML = '₹' + Math.round(tax).toLocaleString()
    }
    catch (err) {
...
const fetchOrder = async () => {
    try {
        ...
        for (let order of data) {
            ...
            const ui = `
                <tr class="border-b">
                    <td class="pr-16 py-4">${moment(order.createdAt).format('DD MMM YYYY hh:mm a')}</td>
                    <td class="py-4 pr-16">
                        <div class="flex items-center gap-3">
                    ...
                    <td class="pr-16 py-4">₹ ${order.price}</td>
                    <td class="pr-16 py-4">${order.discount}%</td>
                    <td class="pr-16 py-4">
                        <h1 class="font-semibold capitalize">${order.user.fullname}</h1>
                    </td>
                    <td class="pr-16 py-4">${order.user.email}</td>
                    <td class="pr-16 py-4">${order.user.mobile}</td>
                    <td class="pr-16 py-4">${order.user.address}</td>
                    <td class="pr-16 py-4">
                        <select class="border border-gray-300 rounded p-2" onchange="updateStatus(this,'${order._id}')">
...
const updateStatus = async (select, id) => {
    if (select.value === 'created') return false
    try {
        const options = {
            headers: {
                Authorization: `Bearer ${token}`
                    ...
                    <td class="py-3 pl-3">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex flex-col">
                                ...
                                <small class="text-gray-600">${moment(user.createdAt).format('DD MMM YYYY hh:mm a')}</small>
                            </div>
                        </div>
                    </td>
                    <td>${user.email}</td>
